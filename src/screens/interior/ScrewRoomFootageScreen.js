...
class ScrewRoomFootageScreen extends React.Component {
    ...
    calcSum(newPrice) {
        ...
        let total_sheets_of_drywall = 0;
        let total_screws = 0;
        for (let i = 0; i < this.state.rooms.length; i++) {
            total_square_footage += AppStyle.checkValue(this.state.rooms[i].total_surface_square_footage);
            total_sheets_of_drywall += AppStyle.checkValue(this.state.rooms[i].total_sheets_of_drywall);
            total_screws += AppStyle.checkValue(this.state.rooms[i].total_screws);
        }
        ...
        this.setState({
            ...
            total_sheets_of_drywall: AppStyle.formatValue(total_sheets_of_drywall, 2),
            total_screws: AppStyle.formatValue(total_screws, 2),
            ...
            return (
                <View style={styles.roomRow} key={index}>
                    ...
                    <View style={styles.resultRow}>
                        ...
                        <Text style={styles.resultValue}>{room.total_surface_square_footage}</Text>
                    </View>
                    <View style={styles.resultRow}>
                        <Text style={styles.resultLabel}>Total Drywall Panels</Text>
                        <Text style={styles.resultValue}>{room.total_sheets_of_drywall}</Text>
                    </View>
                    <View style={styles.resultRow}>
                        <Text style={styles.resultLabel}>Total Screws</Text>
                        <Text style={styles.resultValue}>{room.total_screws}</Text>
                    </View>
                </View>
            )
        });
...
const mapStateToProps = state => ({
    ...
    rooms: state.screwRooms.rooms,
    isLoggedIn: state.auth.isLoggedIn,
});

export default connect(mapStateToProps)(ScrewRoomFootageScreen);
