...
class WallFootageScreen extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            avg_door_width: '',
            number_of_doors: '',
            square_feet_per_gallon_of_paint: '350',
    ...
    onProjects = () => {
        ...
        if (this.props.isLoggedIn) {
            this.props.navigation.dispatch({ type: 'Project', routeName: this.props.navigation.state.routeName, calculation: 'GallonRoomFootage' });
        } else {
            this.props.navigation.dispatch({ type: 'GoToProject', routeName: this.props.navigation.state.routeName, calculation: 'GallonRoomFootage' });
    ...
    onCalc = () => {
        ...
        let square_feet_per_gallon_of_paint = AppStyle.checkValue(this.state.square_feet_per_gallon_of_paint);
        if (!square_feet_per_gallon_of_paint) {
            square_feet_per_gallon_of_paint = 350;
            this.setState({ square_feet_per_gallon_of_paint: AppStyle.formatValue(square_feet_per_gallon_of_paint, 0) });
        }
        ...
        this.setState({
            ...
            gallons_of_paint: AppStyle.formatValue((0.5 + total_surface_square_footage / square_feet_per_gallon_of_paint), 0)
        });
    }
    ...
    onClear = () => {
        this.setState({
            ...
            avg_door_width: '',
            number_of_doors: '',
            square_feet_per_gallon_of_paint: '',
            total_surface_square_footage: '',
            gallons_of_paint: '',
        });
    }
    ...
    createMessage = () => {
        ...
        message += 'Square Feet Per Gallon of Paint = ' + this.state.square_feet_per_gallon_of_paint + '\n\n';

        message += 'Total Surface Square Footage (Minus Windows/Doors) = ' + this.state.total_surface_square_footage + '\n';
        message += 'Gallons of Paint = ' + this.state.gallons_of_paint + '\n';

        return message;
    }

    onEmail = () => {
        AppStyle.onEmail('Wall Footage', this.createMessage());
    }

    onShare = () => {
        AppStyle.onShare('Wall Footage', this.createMessage());
    }

    onSaveRoom = () => {
        const payload = {
            name: 'New Room',
            surface_square_footage: this.state.total_surface_square_footage,
            gallons_of_paint: this.state.gallons_of_paint
        }
        this.props.navigation.dispatch({ type: 'addGallonRoom', newRoom: payload });

        alert("Room calculation saved. See 'All Rooms' for saved values.")
    }

    onViewAllRooms = () => {
        this.props.navigation.navigate('GallonRoomFootage');
    }

    render() {
        return (
            <ScrollView style={styles.container}>
                <Text style={styles.description}>Enter the measurements to calculate the total wall square footage of the room. All measurements in feet.</Text>
                ...
                <Text style={styles.sectionTitle}>Square Feet Per Gallon of Paint (optional)</Text>
                <View style={styles.row}>
                    <CustomInput
                        style={styles.input}
                        inputRef={(c) => this.squareFeetInput = c}
                        label="Square Feet Per Gallon of Paint"
                        number={true}
                        value={this.state.square_feet_per_gallon_of_paint}
                        onChangeText={(text) => { this.setState({ 'square_feet_per_gallon_of_paint': text }) }} />
                </View>
                <View style={styles.row}>
                ...
                <View style={[styles.row, { marginTop: 10 }]}>
                    ...
                    <Text style={styles.resultValue}>{this.state.gallons_of_paint}</Text>
                </View>
                <View style={styles.row}>
...
const mapStateToProps = state => ({
    ...
    gallonRooms: state.gallonRooms.rooms,
    isLoggedIn: state.auth.isLoggedIn,
    prevScreenRoute: state.screen.prevScreenRoute,
});

export default connect(mapStateToProps)(WallFootageScreen);
